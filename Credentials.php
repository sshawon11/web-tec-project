...
//$c = new Credentials();
//$c->createAdminAccount("Md.Samsuddoha", "shawon7582@gmail.com", "shawon@admin");

//PRINT_R($c->loginAdmin("shawon7582@gmail.com", "shawon@admin"));
...
if (isset($_POST['admin_register'])) {
	...
	if (!empty($name)  && !empty($email) && !empty($password) && !empty($cpassword)) {
		...
		echo json_encode(['status'=> 303, 'message'=> 'Invalid!']);
		exit();
	}
}
...
if (isset($_POST['admin_login'])) {
	...
	if (!empty($email) && !empty($password)) {
		...
		echo json_encode(['status'=> 303, 'message'=> 'Indalid!']);
		exit();
